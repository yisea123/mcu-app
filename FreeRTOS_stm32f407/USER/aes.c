...
static unsigned char aes_buf[16] = { 0 };
static unsigned char mStaticKey[ 4 * 15 * 4 ];
static char used = 0;

unsigned char* aes_create_key( unsigned char key[], int len )
{
	unsigned char *w;

	if( used )
	{
		printf("%s: aes is using! please destory first!\r\n", __func__);
		return NULL;
	}
	switch (len) 
	{
	...
	memset( mStaticKey, 0, sizeof( mStaticKey ) );
	w = mStaticKey;
...
void aes_destory_key( unsigned char ** w )
{
	if( *w == mStaticKey )
	{
		*w = NULL;
...
unsigned char* aes_encode_packet(unsigned char* key, unsigned char *pdata, int plength)
{
	if( key != NULL && plength == 16 ) 
	{
		cipher(pdata, aes_buf, key);
		return aes_buf;
	} 
	else 
	{
...
unsigned char* aes_decode_packet(unsigned char* key, unsigned char *pdata, int plength)
{
	if( key != NULL && plength == 16 ) 
	{	
		inv_cipher(pdata, aes_buf, key);
		return aes_buf;	
	} 
	else 
	{	
...
void aes_test(void)
{
	...
	unsigned char *key = NULL, *out	= NULL;
