...
int caculate_file_md5( const char *fileName, unsigned char md5[] )
{
	FIL file;
	unsigned int res = 0;
	unsigned char read[ 20 ];
	unsigned int br;
	MD5_CTX context;  
	...
	while( 1 )
	{
	    res = f_read( &file, read, sizeof( read ), &br );
		if ( res != FR_OK )
		{
		...
		if( br == sizeof( read ) )
		{
			MD5Update( &context, read, br ); 
		}
		else if( br < sizeof( read ) )
		{
			res = 0;
			MD5Update( &context, read, br ); 
			f_close( &file );
	...
	printf("%s:[%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X" \
		   "%02X%02X%02X%02X]\r\n", __func__, md5[0], md5[1], md5[2], 
			md5[3], md5[4], md5[5], md5[6], md5[7], md5[8], md5[9], 
...
int compare_flashdata_md5( unsigned int addrStart, unsigned int size, const unsigned char* md5 )
{
	MD5_CTX context;  
	int result = 0, i;
	...
	printf("%s: addrstart=%x, size=%d\r\n", __func__, addrStart, size);
	
	MD5Init( &context );
	while( readLen < size ) 
	{
		STMFLASH_Read( addrStart, ( u32 * ) d, 1 );
		addrStart += 4;
		readLen += 4;
		
		if( readLen <= size )
		{
			MD5Update( &context, d, 4 ); 
		}
		else
		{
			MD5Update( &context, d, size + 4 - readLen ); 
		}
	} 
...
void caculate_buffer_md5( unsigned char *buffer, int len, char md5[] )
{
	MD5_CTX context;
