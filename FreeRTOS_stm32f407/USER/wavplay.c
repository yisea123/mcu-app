...
u8 wav_play_song(u8* fname)
{
			...
			if(res==0)
			{
				...
				{ 
					...
					while(1)
					{
						//key=KEY_Scan(0); 
						//uMusicPlayControl = 0;
						if(uMusicPlayControl==WKUP_PRES)//暂停
						{
						...
						if((audiodev.status&0X01)==0) vTaskDelay( 20 / portTICK_RATE_MS );
						else break;
					}
				}
				audio_stop(); 
	...
	vPortFree(  audiodev.tbuf);	//释放内存
	vPortFree(  audiodev.i2sbuf1);//释放内存
	vPortFree(  audiodev.i2sbuf2);//释放内存 
	vPortFree(  audiodev.file);	//释放内存 
	return res;
} 
