...
typedef struct tskTaskControlBlock
{
	...
	/*xEventListItem 的pvContainer为NULL, 表示没有阻塞在
	某些事件上，否则阻塞*/
...
/*pxDelayedTaskList 指向了xDelayedTaskList1*/
...
accessed from a critical section. */
			//uxSchedulerSuspended 指示调度器有没有被挂起
...
BaseType_t xReturn;
	...
	if( xReturn == pdPASS )
	{
		...
		xTickCount = ( TickType_t ) ( 0xffffffffU - 100020/ portTICK_RATE_MS );
		...
		if( xConstTickCount >= xNextTaskUnblockTime )
		{
			for( ;; )
			{
				...
				else
				{
					...
					list. */
					/*插入到优先级所在的队列尾巴中
						pxReadyTasksLists[( pxTCB )->uxPriority ]，就绪队列
						有多条，相同的优先级处于相同的
						队列中*/
					prvAddTaskToReadyList( pxTCB );
...
BaseType_t xReturn;
	...
	//如果调度器没有被挂起，把任务放到就绪表中
	if( uxSchedulerSuspended == ( UBaseType_t ) pdFALSE )
	{
		/*一般等待消息的任务都放在延时队列中，
		这里的pxUnblockedTCB->pvContainer 就是延时队列*/
	...
	else
	{
		...
		//调度器被挂起时， 就绪队列跟延时队列不可入
		//把任务放到xPendingReadyList 队列中
...
/*延时队列发生了变化，需要更新下个
unblock的tick数 xNextTaskUnblockTime，如果没有任务在
延时队列上，xNextTaskUnblockTime 则设置为最大延时
portMAX_DELAY*/
static void prvResetNextTaskUnblockTime( void )
{
TCB_t *pxTCB;
