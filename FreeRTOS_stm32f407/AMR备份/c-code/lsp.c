...
int lsp_init (lspState **st)
{
  ...
  if (st == (lspState **) NULL){
      printf( "lsp_init: invalid parameter\n");
      return -1;
  }
  ...
  *st = NULL;
  ...
  /* allocate memory */
  if ((s= (lspState *) pvPortMalloc(sizeof(lspState))) == NULL){
      printf( "lsp_init: can not malloc state structure\n");
      return -1;
  }
...
int lsp_reset (lspState *st)
{
  ...
  if (st == (lspState *) NULL){
      printf( "lsp_reset: invalid parameter\n");
      return -1;
  }
