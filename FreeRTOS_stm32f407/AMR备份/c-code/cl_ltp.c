...
int cl_ltp_init (clLtpState **state)
{
    ...
    if (state == (clLtpState **) NULL){
        printf( "cl_ltp_init: invalid parameter\n");
        return -1;
    }
    *state = NULL;
    
    /* allocate memory */
    if ((s= (clLtpState *) pvPortMalloc(sizeof(clLtpState))) == NULL){
        printf(  "cl_ltp_init: can not malloc state structure\n");
        return -1;
  }
...
{
    if (state == (clLtpState *) NULL){
        printf( "cl_ltp_reset: invalid parameter\n");
        return -1;
    }
