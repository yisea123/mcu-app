...
u8 RTC_INIT(void)
{
	...
	if(RTC_Read_BKR(0)!=0X505a)		//是否第一次配置?
	{
		RCC->BDCR|=1<<0;			//LSE 开启  
		...
		RTC->ISR&=~(1<<7);			//退出RTC初始化模式
		RTC->WPR=0xFF;				//使能RTC寄存器写保护  
		RTC_Set_Time(22,10,45,0);	//设置时间
		RTC_Set_Date(16,9,26,1);		//设置日期
		//RTC_Set_AlarmA(7,0,0,10);	//设置闹钟时间
		RTC_Write_BKR(0,0X505a);	//标记已经初始化过了
	} 
	else
	{
