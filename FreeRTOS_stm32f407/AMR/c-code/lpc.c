...
int lpc_init (lpcState **state)
{
  ...
  if (state == (lpcState **) NULL){
      printf( "lpc_init: invalid parameter\n");
      return -1;
  }
  *state = NULL;
 
  /* allocate memory */
  if ((s= (lpcState *) pvPortMalloc(sizeof(lpcState))) == NULL){
      printf( "lpc_init: can not malloc state structure\n");
      return -1;
  }
...
int lpc_reset (lpcState *state)
{
  ...
  if (state == (lpcState *) NULL){
      printf( "lpc_reset: invalid parameter\n");
      return -1;
  }
