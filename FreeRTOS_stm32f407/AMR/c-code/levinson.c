...
int Levinson_init (LevinsonState **state)
{
  ...
  if (state == (LevinsonState **) NULL){
      printf( "Levinson_init: invalid parameter\n");
      return -1;
  }
  *state = NULL;
 
  /* allocate memory */
  if ((s= (LevinsonState *) pvPortMalloc(sizeof(LevinsonState))) == NULL){
      printf( "Levinson_init: can not malloc state structure\n");
      return -1;
  }
...
int Levinson_reset (LevinsonState *state)
{
  ...
  if (state == (LevinsonState *) NULL){
      printf( "Levinson_reset: invalid parameter\n");
      return -1;
  }
