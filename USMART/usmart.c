...
{ 		    		  			    
	if(TIM_GetITStatus(TIM4,TIM_IT_Update)==SET)//溢出中断
	{
		...
		TIM_SetCounter(TIM4,0);		//清空定时器的CNT
		TIM_SetAutoreload(TIM4,6000);//恢复原来的设置		 
		/*从100改为400*/		
	}				   
...
#if USMART_ENTIMX_SCAN==1
	/*从1000改为4000*/
	Timer4_Init(6000,(u32)sysclk*100-1);
			...
			if(sta)
			{
				switch(sta)
				{
					...
					case USMART_PARMOVER:
						printf("%s:para too more!\r\n", __func__);   			
						break;		
					case USMART_NOFUNCFIND:
						printf("%s:can not find the function!\r\n", __func__);   			
						break;		
				}
			}
		}
