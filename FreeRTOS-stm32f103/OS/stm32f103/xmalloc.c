...
{  
    xmemset(mallco_dev.memmap[memx], 0,memtblsize[memx]*2);//内存状态表数据清零  
	xmemset(mallco_dev.membase[memx], 0,memsize[memx]);	//内存池所有数据清零  
...
{  
    ...
    if (offset == 0XFFFFFFFF) {
    	...
    	vTaskSuspendAll();
	    xmemcpy((void*)((u32)mallco_dev.membase[memx]+offset), ptr, size);	//拷贝旧内存内容到新内存   
        xfree(memx, ptr);  											  		//释放旧内存
		( void )xTaskResumeAll();
